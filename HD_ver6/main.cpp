...
int dem1=0;
int dem2=0;
int bienGiaTriThayDoi[2];
int bienGiaTri[5] = {0,0,0,0,0};
...
int main(){
	...
	for (;;){
		...
		bienGiaTriXet[0] = hg1.mostFrequentFingerNumber;
		...
		bienGiaTriXet[1] = hg1.mostFrequentFingerNumber;

		

		if (bienGiaTriXet[0] == bienGiaTriXet[1]){
			dem1++;
		}else{
			bienGiaTriThayDoi[0] = bienGiaTriXet[0];
			bienGiaTriThayDoi[1] = bienGiaTriXet[1];
		}
		...
		if (dem2 == 6){
			...
			if (dem1 == dem2){
				...
				hienthi = "";
				cout << "thay doi" << endl;
				if (bienGiaTriThayDoi[0] != 0 
					&& bienGiaTriThayDoi[1] == 0){
					cout << "den so " << bienGiaTriThayDoi[0] << " tat" << endl;
					bienGiaTri[bienGiaTriThayDoi[0]-1] = 0;
					hienthi = "den so " + bienGiaTriThayDoi[0];
					hienthi = hienthi + " tat";
				}
				if (bienGiaTriThayDoi[0] == 0 
					&& bienGiaTriThayDoi[1] != 0){
					cout << "den so " << bienGiaTriThayDoi[1] << " bat" << endl;
					bienGiaTri[bienGiaTriThayDoi[1]-1] = 1;
					hienthi = "den so" + bienGiaTriThayDoi[1];
					hienthi = hienthi + " bat";
				}
				if (bienGiaTriThayDoi[0] != 0 
					&& bienGiaTriThayDoi[1] != 0 
					&& bienGiaTriThayDoi[0] != bienGiaTriThayDoi[1]){
					cout << "den so " << bienGiaTriThayDoi[1] << " bat" << endl;
					bienGiaTri[bienGiaTriThayDoi[1]-1] = 1;
					hienthi = "den so" + bienGiaTriThayDoi[1];
					hienthi = hienthi + " bat";
			...
			//putText(m.src, hienthi , Point(40, 40), fontFace, 2.0f, Scalar(255, 255, 0));
			//waitKey(30);
			bienGiaTriThayDoi[0] = 0;
			bienGiaTriThayDoi[1] = 0;
			dem1 = 0;
			dem2 = 0;
		}
...
void makeContours(MyImage *m, Shape * sh, HandGesture *hg1, HandGesture *hg2){
	...
	if (sh->cIdxShape != -1){
		...
		if (currentArea > 5000){
			analyContour(m, sh, sh->cIdxShape, 0/*sh->frNumber*/, hg1);
		}
	}
