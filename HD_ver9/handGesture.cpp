...
vector<Point> HandGesture::getFingerTipsLinux(MyImage *m){
	fingerTips.clear();
	int i = 0;
...
vector<Point> HandGesture::getFingerTipsWindows(MyImage *m){
	fingerTips.clear();
	int i = 0;
	vector<Vec4i>::iterator d = defects.begin();
	while (d != defects.end()) {
...
#ifdef __linux__
	return getFingerTipsLinux(m);

#endif

#ifdef _WIN32
	return getFingerTipsWindows(m);
#endif
}
